...
class Contract(models.Model):
    ...
    def _compute_structure_type_id(self):
        ...
        for contract in self:
            if (
                not contract.structure_type_id
                or contract.structure_type_id.country_id
                != contract.company_id.country_id
            ):
                contract.structure_type_id = _default_salary_structure(
                    contract.company_id.country_id.id
                )
    ...
    def update_state(self):
        ...
        for contract in work_permit_contracts:
            contract.with_context(mail_activity_quick_update=True).activity_schedule(
                ...
                contract.date_end,
                _(
                    "The work permit of %s is about to expire.",
        ...
        if work_permit_contracts:
            work_permit_contracts._safe_write_for_cron(
    ...
    def write(self, vals):
        ...
        for contract in self:
            if (
                contract == contract.sudo().employee_id.contract_id
        ...
        if calendar:
            ...
            ).mapped("employee_id").filtered(lambda e: e.resource_calendar_id).write(
